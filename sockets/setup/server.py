...
oled.shutter_display(['Mentioner!','Starting.','Starting..','Starting.','Connecting to Wifi'],'center',1)

nets = net.net(ssid=WIFI_NAME,password=WIFI_PASS)

oled.shutter_display([f'Connected with {nets.connection_name()}','Starting socket.','Starting socket...','Starting socket.'],'center',1)
...
def handle_client(conn, addr):
   ...
   while True:
      ...
      if msg_len:
         if len(mentions) == 0: oled.clear_display()
         msgg = conn.recv(int(msg_len)).decode(FORMAT)
         msg = msgg.split()
         # TODO : Scroll feature
         if len(mentions) > 8: print('Mention limit reached') ; sys.exit()
         blink(RELAY,1) ; blink(LED2,1)
         if msg[0] not in mentions.keys():
            mentions.update({msg[0]:[1,y]})
            y+=8
         else:
            mentions.update({msg[0]:[mentions[msg[0]][0]+1,mentions[msg[0]][1]]})
